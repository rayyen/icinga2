...
Required parameters:
  ...
  -m MSMTPACCOUNTID (\$notification.vars.msmtp_account\$)
  -p PARENT_NODE
...
## Main
while getopts 4:6:b:c:d:e:f:hi:m:l:n:o:r:s:t:u:p:v: opt
do
  case "$opt" in
...
if [ -n "$MAILFROM" ] ; then
  ...
  if [ -f /etc/debian_version ]; then
    echo $USEREMAIL
    echo "MSMTPACCOUNTID="$MSMTPACCOUNTID
    echo -e "Subject: $SUBJECT\r\n\r\n$NOTIFICATION_MESSAGE" | /usr/bin/msmtp $USEREMAIL --account=$MSMTPACCOUNTID
